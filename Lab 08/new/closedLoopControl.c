...
int MoveMotor (int fd, float distance, int motor)
{
    ...
    while (n == 7) {	// 7 is just a number to loop the while continously
		usleep(10);
...
int MoveMotorRectangular (int fd, float distance, int steps, int useVision, int camIndex)
{
	...
	////////////////////////////////////////////
     ...
     // in the .txt file
        ...
        ColorTracking (frame, &xc , &yc, cvScalar(115,113,56), cvScalar(167,205,215));
...
int PID (int fd, int targetPositionX, int targetPositionY, CvCapture* capture)
{
    ...
    float ki_x = 0.1, ki_y = 0.1;
    float kd_x = 0.2, kd_y = 0.2;
    
    int iter=0;
    int c; 	// for wait key
    float dt = 0,timestamp=0;

    // we need the calibration constant to convert from pixels to mm 
    float cal = 15.8;    // insert correct value found in previous task
	toll_dist = toll/cal;
	...
	FILE *fp;
	IplImage* frame;
	CvScalar min=cvScalar(115,113,56), max=cvScalar(167,205,215);
    ...
    while (e_x>toll || e_y>toll || e_x<-toll || e_y<-toll) {
		...
		//~ dt = (currentTime.tv_nsec-prevTime.tv_nsec)/1000000000.00;  // time in microseconds
		printf("\ndt = %f\n",dt);
